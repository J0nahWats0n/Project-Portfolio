...
# prepare a text file to store the profiling data and response rates of each choice for each survey question
...
with open(response_rates, 'w') as f:
    ...
    for column in df.columns:      
        """This for loop counts the number of responses per column in the dataset by looping through every column in the dataset, 
        counting the frequency of each unique value (including missing values), and matching numeric codes to descriptive labels
        in the JSON file, where the results are then written to a text file."""
        ...
        # this is the appropriate way to match variable names based on the way I renamed columns in my dataframe
        # example:  "N3Q1": "N3Q1: Overall health rating" becomes "N3Q1"

        if variable_name in labels:
        # my JSON file (labels) contains all of the data prior to cleaning, so some labels won't be used (that's why I use "if")
